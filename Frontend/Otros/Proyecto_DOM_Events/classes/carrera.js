class Carrera {
  ...
  addCatedra(unaCatedra) {
    if (unaCatedra && !this.catedras.find((element) => element.id === unaCatedra.id)) {
  ...
  addMatricula(unaMatricula) {
    if (unaMatricula && !this.matriculados.find((element) => element.id === unaMatricula.id)) {
  ...
  isExisteMatricula(legajo) {
    return this.matriculados.some( (matricula) => matricula.estudiante.id === legajo );
  }
}
