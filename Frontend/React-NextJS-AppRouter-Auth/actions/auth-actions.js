...
export async function signup(prevState, formData) { // store it in the database (create a new user)
    const email = formData.get('email');
    const password = formData.get('password');
    let errors = {};
    
    if (!email.includes('@')) errors.email = 'Please enter a valid email address.';
    if (password.trim().length < 6) errors.password = 'Password must be at least 8 characters long.';
