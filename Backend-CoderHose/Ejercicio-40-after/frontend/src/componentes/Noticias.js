...
import './Noticias.css'
import generarNoticia from '../generador.js'
            ...
            <div className="Noticias">
                <div className="container mt-3">
                    <div className="jumbotron">
                        ...
                        <button className="btn btn-info my-3 float-left" onClick={ () => this.obtenerNoticias(idObtenerNoticia)}><i className="fas fa-file-alt"> Obtener</i></button>
                        <input value={idObtenerNoticia} className="mt-3 ml-1 form-control w-25 float-left" onChange={e => this.setState({ idObtenerNoticia: e.target.value })} placeholder="todas ó ingrese ID" type="text" />
                        
                        <button className="btn btn-success my-3 float-right" onClick={ () => this.incorporarNoticia()} >
                            <i className="fas fa-envelope-open-text"> Generar</i>
                        </button>

                        <div className="clearfix" />
                        {
                            !noticias.length && pedidas && <h3 className="alert alert-danger"> No hay noticias para mostrar </h3>
                        }

                        {
                            noticias.map(function (noticia, index){
                                return  <Noticia noticia={noticia} index={index} marcarLeida={_id => this.actualizarComoLeida(_id)} borrar={_id => this.borrarNoticia(_id)} key={noticia._id}/>
                            })
                        }
                    </div>
                </div>
