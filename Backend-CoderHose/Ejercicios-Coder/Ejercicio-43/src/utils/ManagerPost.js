...
class ManagerPost {
  ...
  async savePost(post) {
    if (!post) return console.log("El post está vacío");

    const existsPost = this.posts.find((event) => event.id === post.id);
    if (existsPost) {
  ...
  async deletePost(id) {
    const post = this.posts.find((event) => event.id === id);
    if (!post) throw Error("El post no existe");

    const index = this.posts.findIndex((event) => event.id === id);
