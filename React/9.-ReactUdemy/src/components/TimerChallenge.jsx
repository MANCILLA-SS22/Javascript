...
function TimerChallenge({title, targetTime}) {
    const [timeStarted, setETimeStarted] = useState(false);
    const [timeExpired, setETimeExpired] = useState(false);
    
    // Every component instance of TimerChallenge component will get its own timer ref that works totally independent from the other refs that belong to the other instance of that 
    // component. Unlike varaible defined in component functions, this ref will not be reset or cleared when this component re-executes. Instead, just as with state values, React 
    // will store these timer values behind the scenes and and make sure that they don't get lost as this component function re-executes. 
    const timer = useRef(); 
    const dialog = useRef();

    function handleStart(){
        timer.current = setTimeout(() => {
            setETimeExpired(true); //This line is executed once the timer expire (after 1 second).
            dialog.current.showModal(); //showModal() is a function it comes out from <dialog>
        }, targetTime*1000);

        setETimeStarted(true); //This line is first executed after setting the timer (setTimeout), not after it expired.
    }
