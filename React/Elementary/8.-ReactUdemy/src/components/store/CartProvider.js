...
function CartReducer (state, action){  //state = initial state   <>   action = parameter into the dispatch function (or new item)
    ...
    if (action.type === "ADD") { 
        ...
        if (!existingCartItem) uptdatedItems = state.items.concat(action.item); //Concat joins arrays and returns an array with the joined arrays. Here, our inital value (the old one) is into state, and the value that will be concatened to it, will be the one into "action".
    ...
    if (action.type === "REMOVE") {
        ...
        if (existingItem.amount === 1) updatedItems = state.items.filter(event => event.id !== action.id); 
        const updatedItem = {
            ...existingItem,
