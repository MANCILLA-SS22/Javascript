...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    addItemToCart(state, action) {
      ...
      state.totalQuantity++;
      if (!existingItem) state.items.push({ id: newItem.id, price: newItem.price, quantity: 1, totalPrice: newItem.price, name: newItem.title, });
...
function sendCartData(cart){
  return async function(dispatch){
    ...
    const sendRequest = async () => {
      const response = await fetch( 'https://react-http-6b4a6.firebaseio.com/cart.json', { method: 'PUT', body: JSON.stringify(cart) } );
...
export { sendCartData, cartActions, cartReducer }