...
function generateProductErrorInfo(data){
    ...
    return `
        Alguno de los datos son inválidos. La lista de propiedades requeridas son:
            *Título: Se esperaba un string, se recibió: ${data.title}
            *Description: Se esperaba un string, se recibió: ${data.description}
            *Price: Se esperaba un number, se recibió: ${data.price}
            *Thumbail: Se esperaba un string, se recibió: ${data.thumbnail}
            *Code: Se esperaba un string, se recibió: ${data.code}
            *Stock: Se esperaba un number, se recibió: ${data.stock}
            *Category: Se esperaba un string, se recibió: ${data.category}
    `;
